...
`RemoteBackend`（JSON-RPC 経由）。native の Send フューチャ前提（egui は具象型保持で `runtime.spawn`）。
...
- **Phase 2** 🟡: framework 側 `sapphire-framework-backend`（非同期 `WorkspaceBackend` + `BackendEvent`
  + `LocalBackend`/`RemoteBackend`）✅。**残: journal desktop GUI を `JournalBackend` 経由へリファクタ（別リポジトリ・別 PR）**。
