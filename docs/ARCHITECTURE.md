...
- **既定実装 = `RedbStore`（redb + tantivy + brute-force vectors）**。C依存ゼロ・純Rust。
...
- **feature**: `redb-store`（既定）/ `sqlite-store`（optional-legacy、将来削除）/ `lancedb-store` / `fastembed-embed`。
  `VectorDb` config enum に `Redb` を追加（既定のブルートフォース）。
...
- **Phase 0c**（キャッシュ SQLite 脱却）✅: `RedbStore`(redb+tantivy+brute-force) を既定に。sqlite-store は optional-legacy。
- **Phase 1** ⬜: 旧 `sapphire-workspace` を re-export shim/アーカイブ化。`.gitmodules` 更新。journal/agent/ledger の依存を
  framework へ差し替え。`sapphire-framework-retrieve` の DocStore/VectorStore 論理分離の仕上げ。journal `cache.rs` の redb 化。
