...
```toml
[[key]]
token = "sjt_9f3aK2…"        # 必須。プレフィクスはアプリが指定する
...
- **トークンの形式**は `<prefix>_<random>`。区切りはアンダースコア（`syt_` / `ghp_` /
  `sk_live_` と同じ多数派であり、`_` は単語構成文字なのでダブルクリックでトークン全体が
  選択される。`-` だと選択がそこで切れる）。乱数部は `getrandom` で 32 バイト取り、
  既存の `base64` 依存の URL-safe / no-pad で符号化する（43 文字、新規依存なし）。
