...
`device_id` **フィールド**の追加自体は既存の鍵ファイルと後方互換 — `device_id` を知らない時代に
書かれた `[[key]]`（フィールドが無い）もそのまま読める。registry のファイル 2 枚は新規で、
存在しなければ空として扱う（`KeyStore::load` と同じ — 作成はしない）。

一方 `KeyStore::generate` の**シグネチャ**は破壊的変更である。`device_id: Option<GrainId>` が
3 番目の位置引数として挿入され、4 引数から 5 引数になった。対応するコミットは
`feat(remote-server)!` として明示的にマークしてある。`generate` を呼んでいる既存の利用者 —
**`sapphire-journal-server`** と **`sapphire-agent`** — はこのブランチに対してそのままでは
コンパイルが通らない。それぞれの呼び出し箇所に `device_id` 引数（多くの場合 `None`）を挿入する
追従が要る。「framework の既存利用者に破壊的変更は無い」は誤りだった — フィールドは加法的でも、
関数シグネチャは加法的ではない。
