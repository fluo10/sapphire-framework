...
# id          optional. A 7-character grain-id. Filled in on load when blank.
#             Used to tie a key to a user, so it survives a label change.
...
impl KeyStore {
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        // id は revoke と設定ファイルからの参照に使う手掛かりなので、重複したまま
        // 読み込むと「どちらの鍵か」が決まらない。7 文字になった以上、人は手で
        // 書く。UUID では現実的でなかった重複が起こりうるのでここで落とす。
        //
        // 補完より先に集めておくのは、補完で引いた値が同じファイルの別の行に
        // 書かれている id とぶつからないようにするため。
        ...
        for k in raw.key {
            ...
            let id = match k.id {
                Some(id) => id,
                None => {
                    // 時刻由来ではなく乱数。デシ秒精度の時刻由来だと近接生成で
                    // 上位桁が揃い、7 文字しかない id が読み分けづらくなる。
                    let id = fresh_id(&taken, GrainId::random)?;
                    taken.insert(id);
                    id
                }
            };
    ...
    pub fn generate(
        ...
        let taken: HashSet<GrainId> = self.entries.iter().map(|e| e.id).collect();
        let entry = KeyEntry {
            token: format!("{prefix}_{random}"),
            id: fresh_id(&taken, GrainId::random)?,
            label,
            created_at: Utc::now(),
            expires_at,
        };
    ...
    ///
    /// **`id` 一致が label 一致より先**。grain-id は 7 文字なので `laptop1` の
    /// ような文字列も文法上は id として読め、id と label の名前空間は UUID の
    /// ころより近い。どちらが勝つかは偶然ではなく仕様として決めておく。
...
/// `taken` に無い id を `mint` から引き当てる。
///
/// 35 ビットの空間に鍵ファイルの中身はごく少数なので、実際に衝突することは
/// まずない。それでも引き直すのは、生成側が既存の id を全部手元に持っている
/// 以上、衝突の確率を小さくするのではなく衝突そのものを消せるから。
fn fresh_id(taken: &HashSet<GrainId>, mut mint: impl FnMut() -> GrainId) -> Result<GrainId> {
    const ATTEMPTS: usize = 64;
    (0..ATTEMPTS)
        .map(|_| mint())
        .find(|id| !taken.contains(id))
        .ok_or_else(|| Error::KeyFile(format!("no free id after {ATTEMPTS} attempts")))
}
...
mod tests {
    ...
    fn load_fills_in_a_hand_written_entry_and_writes_it_back() {
        ...
        let id = store.entries()[0].id.to_string();
        assert_eq!(id.len(), 7, "鍵の id は 7 文字の grain-id: {id}");
        assert_eq!(
            id.parse::<GrainId>().unwrap(),
            store.entries()[0].id,
            "表示形から読み戻せる"
        );
    ...
    fn taken(ids: &[&str]) -> HashSet<GrainId> {
        ids.iter().map(|s| s.parse().unwrap()).collect()
    }

    #[test]
    fn fresh_id_skips_an_id_that_is_already_taken() {
        // 35 ビットは確率的には十分だが、生成側は全 id を手元に持っている。
        // 確率に頼らず引き直して衝突を消せる。
        let mut seq = ["123abcd", "0000001"]
            .into_iter()
            .map(|s| s.parse::<GrainId>().unwrap());

        let id = fresh_id(&taken(&["123abcd"]), || seq.next().unwrap()).unwrap();

        assert_eq!(id.to_string(), "0000001");
    }

    #[test]
    fn fresh_id_gives_up_instead_of_looping_forever() {
        let always_taken = || "123abcd".parse::<GrainId>().unwrap();

        let err = match fresh_id(&taken(&["123abcd"]), always_taken) {
            Ok(id) => panic!("空いていない id を返してはならない: {id}"),
            Err(e) => e.to_string(),
        };

        assert!(err.contains("id"), "何に失敗したか示す: {err}");
    }

    #[test]
    fn load_rejects_two_keys_that_share_an_id() {
        // id が 7 文字になった以上、人は手で書く。UUID では現実的でなかった
        // 重複が起こりうるので、黙って受けずに落とす。
        let tmp = tempfile::tempdir().unwrap();
        let p = path(&tmp);
        std::fs::write(
            &p,
        ...
        assert!(err.contains("123abcd"), "どの id が重複したか示す: {err}");
    }
    ...
    #[test]
    fn revoke_matches_an_id_before_a_label_that_looks_like_one() {
        let tmp = tempfile::tempdir().unwrap();
        let mut store = KeyStore::load(&path(&tmp)).unwrap();
        let target = store.generate("sjt", Some("by id".into()), None).unwrap();
        // 別の鍵の id をそのままラベルに持つ鍵。7 文字なら手で書ける以上ありうる。
        let decoy = store
        ...
        assert_eq!(removed.id, target.id, "id 一致が label 一致より先");
        assert_eq!(store.entries().len(), 1);
